...
@section('title', translate('Service Expense Report'))
    ...
    <div class="content container-fluid">
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                <span>
                    {{ translate('Service Expense Report') }}
                </span>
            </h1>
        </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-sm-6 col-md-3">
                            <select class="form-control js-select2-custom set-filter" data-url="{{ url()->full() }}" data-filter="type" name="type">
                                ...
                                @php
                                    // Created via BookingLogicTrait::bookingExpenseCreate (service_booking_id set).
                                    $serviceTypes = [
                                        'CashBack'            => 'messages.CashBack',
                                        'coupon_discount'     => 'messages.coupon_discount',
                                        'discount_on_booking' => 'messages.discount_on_booking',
                ...
                <div class="search--button-wrapper">
                    ...
                    <form class="search-form theme-style">
                        <div class="input--group input-group input-group-merge input-group-flush">
                            <input name="search" type="search" value="{{ request()?->search ?? null}}" class="form-control" placeholder="{{ translate('Search by Booking ID') }}">
                    ...
                    <div class="hs-unfold ml-3">
                        ...
                        <div id="usersExportDropdown" class="hs-unfold-content dropdown-unfold dropdown-menu dropdown-menu-sm-right hs-unfold-content-initialized hs-unfold-css-animation animated hs-unfold-reverse-y hs-unfold-hidden">
                            ...
                            <a id="export-excel" class="dropdown-item" href="{{route('admin.transactions.report.service-expense-export', ['export_type'=>'excel',request()->getQueryString()])}}">
                                <img class="avatar avatar-xss avatar-4by3 mr-2" src="{{ asset('public/assets/admin') }}/svg/components/excel.svg" alt="Image Description">
                                {{ translate('messages.excel') }}
                            </a>
                            <a id="export-csv" class="dropdown-item" href="{{route('admin.transactions.report.service-expense-export', ['export_type'=>'csv',request()->getQueryString()])}}">
                    ...
                    <table class="table table-borderless middle-align __txt-14px">
                        ...
                        <tbody id="set-rows">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($exp->serviceBooking)
                                        <div>
                                            <a class="text-dark" href="{{ route('admin.service.booking.details', $exp->serviceBooking->id) }}">{{ $exp['service_booking_id'] }}</a>
                                        </div>
                                    @else
                                ...
                                <td class="text-center">
                                    @if ($exp?->serviceBooking?->customer)
                                        {{ $exp?->serviceBooking?->customer?->fullName ?? ($exp?->serviceBooking?->customer?->f_name . ' ' . $exp?->serviceBooking?->customer?->l_name) }}
                                    @elseif(isset($exp?->serviceBooking?->user_info['contact_person_name']))
                                        <div class="font-medium">{{$exp?->serviceBooking?->user_info['contact_person_name'] }}</div>
                                    @else
